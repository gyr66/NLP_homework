...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        # Split train to get a validation part
        train_splited = raw_datasets["train"].train_test_split(test_size=0.15)
        raw_datasets["train"] = train_splited["train"]
        raw_datasets["validation"] = train_splited["test"]
    ...
    if is_regression:
        ...
        for split in raw_datasets.keys():
            if raw_datasets[split].features["label"].dtype not in [
                "float32",
                "float64",
            ]:
                logger.warning(
        ...
        if (
            raw_datasets["train"].features["label"].dtype == "list"
    ...
    model.config.e1_start_token_id = tokenizer.convert_tokens_to_ids("<e1>")
    model.config.e2_start_token_id = tokenizer.convert_tokens_to_ids("<e2>")
    model.config.e1_end_token_id = tokenizer.convert_tokens_to_ids("</e1>")
    model.config.e2_end_token_id = tokenizer.convert_tokens_to_ids("</e2>")
